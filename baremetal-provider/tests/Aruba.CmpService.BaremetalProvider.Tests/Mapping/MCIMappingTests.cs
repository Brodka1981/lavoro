...
public class MCIMappingTests : TestBase
{
    public MCIMappingTests(ITestOutputHelper output) : base(output)
    { }
    ...
    public void MapToResponse_Dto_Full()
    {
        ...
        ret.Services[0].MCIServiceType.Should().Be(BundleServiceModuleType.Server);
        ret.Services[3].MCIServiceType.Should().Be(BundleServiceModuleType.Firewall);
    }
    ...
    {
        MCI input = CreateMCI(false, true);
        var ret = input.MapToResponse();
        ret.Should().NotBeNull();
    ...
    {
        var input = CreateLegacyMCIListItem();
        ...
        ret.Category.Typology.Id.Should().Be(Typologies.MCI.Value);
        ret.Category.Typology.Name.Should().Be(Typologies.MCI.Value.ToUpperInvariant());
    ...
    {
        var input = CreateLegacyMCIListItem(true);
        var legacyProject = CreateLegacyProject();
        var location = CreateLocation();
        var ret = input.MapToListitem("ARU-25198", legacyProject, location);
    ...
    {
        var input = CreateLegacyMCIListItem(false, true);
        var legacyProject = CreateLegacyProject();
        var location = CreateLocation();
        var ret = input.MapToListitem("ARU-25198", legacyProject, location);
    ...
    {
        var input = CreateLegacyMCIDetail();
    ...
    {
        var input = CreateLegacyMCIDetail(true);
        var ret = input.MapToDetail(CreateLocation(), CreatePRJ());
    ...
    {
        var input = CreateLegacyMCIDetail(false, true);
        var ret = input.MapToDetail(CreateLocation(), CreatePRJ());
    ...
    public void MapToListResponse_Content()
    {
        ...
        ret.Values[0].MCIServiceType.Should().Be(BundleServiceModuleType.Server);
        ret.Values[3].MCIServiceType.Should().Be(BundleServiceModuleType.Firewall);
    }
    ...
    private static MCI CreateMCI(bool emptyFolder = false, bool nullFolder = false)
    {
        return new MCI()
        {
            Id = "123",
            ...
            Properties = CreateMCIProperties(emptyFolder, nullFolder),
            NumServices = 4,
            Services = CreateMCIContent(),
            MonthlyUnitPrice = 10
        };
    }
    ...
    private static MCIProperties CreateMCIProperties(bool emptyFolder = false, bool nullFolder = false)
    {
        var ret = new MCIProperties()
        {
    ...
    private static List<BundleContent> CreateMCIContent()
    {
        var ret = new List<BundleContent>();

        ret.Add(new BundleContent()
        {
            ServiceID = 1,
    ...
    private static List<LegacyBundleService> CreateLegacyMCIContent()
    {
    ...
    private LegacyMCIListItem CreateLegacyMCIListItem(bool emptyFolder = false, bool nullFolder = false)
    {
        var ret = new LegacyMCIListItem() 
        { 
            Id = 1,
            Name = "Name",
    ...
    private LegacyMCIDetail CreateLegacyMCIDetail(bool emptyFolder = false, bool nullFolder = false) 
    {
        var ret = new LegacyMCIDetail()
        {
            Id = 1,
            Status = MCIStatuses.Active,
            MonthlyUnitPrice = 50,
            ...
            IncludedInFolders = ["Folder1"],
            Name = "Name",
            BundleContent = CreateLegacyMCIContent(),
        };
