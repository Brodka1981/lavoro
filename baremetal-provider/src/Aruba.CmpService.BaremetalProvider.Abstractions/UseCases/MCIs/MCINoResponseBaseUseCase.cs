...
public abstract class MCINoResponseBaseUseCase<TRequest, TResponse> : NoResponseBaseUseCase<TRequest, TResponse>
    where TRequest : BaseUseCaseRequest
    where TResponse : EmptyMessageBusResponse
{
    protected IMCIsService MCIsService { get; }

    protected MCINoResponseBaseUseCase(IMCIsService mcisService)
    {
        MCIsService = mcisService;
    }
