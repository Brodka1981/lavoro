...
public abstract class ServerNoResponseBaseUseCase<TRequest, TResponse> : NoResponseBaseUseCase<TRequest, TResponse>
    where TRequest : BaseUseCaseRequest
    where TResponse : EmptyMessageBusResponse
{
    protected IServersService ServersService { get; }

    protected ServerNoResponseBaseUseCase(IServersService serversService)
    {
