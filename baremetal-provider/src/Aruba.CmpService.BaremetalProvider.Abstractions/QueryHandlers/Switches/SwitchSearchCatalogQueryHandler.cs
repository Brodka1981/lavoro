...
public class SwitchSearchCatalogQueryHandler : IQueryHandler<CatalogFilterRequest, SwitchCatalog>
{
    private readonly ISwitchesService _switchesService;

    public SwitchSearchCatalogQueryHandler(ISwitchesService switchesService)
    {
        _switchesService = switchesService;
    }

    public async Task<SwitchCatalog> Handle(CatalogFilterRequest request)
    {
        ParametersCheck(request);

        var catalog = await _switchesService.SearchCatalog(request, CancellationToken.None).ConfigureAwait(false);
