...
public class ServerSearchCatalogQueryHandler : IQueryHandler<CatalogFilterRequest, ServerCatalog>
{
    private readonly IServersService serversService;

    public ServerSearchCatalogQueryHandler(IServersService serversService)
    {
        this.serversService = serversService;
    }

    public async Task<ServerCatalog> Handle(CatalogFilterRequest request)
    {
        ParametersCheck(request);

        var catalog = await serversService.SearchCatalog(request, CancellationToken.None).ConfigureAwait(false);
